...
Options:
  ...
  --encrypt-rootpart        Encrypt root partition during flashing
  --encrypt-datapart        Encrypt data partition during flashing
  --dockerapp-provision     Provision docker application image, args format: repository:tag,username,password,server
...
# Check sudoer permission
if [ "$(id -u)" != "0" ]; then
    echo "This script must be run as a sudoer!"
    exit 1
fi

# Check Ubuntu/Debian OS
if ! grep -Eiq "ubuntu|debian" /etc/issue; then
    echo "This script must be run on a Ubuntu/Debian OS!"
    exit 1
fi
...
if [ -z "$(which sgdisk)" ]; then
    PACKAGES="$PACKAGES gdisk"
fi

if [ -z "$(which udisksctl)" ]; then
    PACKAGES="$PACKAGES udisks2"
fi

if [ -z "$(which bmaptool)" ]; then
    PACKAGES="$PACKAGES bmap-tools"
fi

if [ -z "$(which dtc)" ]; then
    PACKAGES="$PACKAGES device-tree-compiler"
fi

if [ -n "$PACKAGES" ]; then
    echo "Missing packages: $PACKAGES, they are being required to run this script, please install them."
    exit 1
fi
...
encrypt_rootpart=0
encrypt_datapart=0
dockerapp_provision=0
dockerapp_provision_args=

ARGS=$(getopt -n $(basename "$0") -l "usb-instance:,help,skip-bootloader,erase-nvme,encrypt-rootpart,encrypt-datapart,dockerapp-provision:" -o "u:v:h" -- "$@")
if [ $? -ne 0 ]; then
    usage >&2
    exit 1
fi
...
while true; do
	...
	--erase-nvme)
        ...
        --encrypt-datapart)
            ...
            ;;
	--dockerapp-provision)
	    dockerapp_provision=1
	    dockerapp_provision_args="$2"
	    shift 2
	    ;;
	-u)
	    keyfile="$2"
...
mount_partition() {
    ...
    # Try to force umount $dev firstly, this ensures it's umounted
    # when it's on busy.
    if cat /proc/mounts | grep -q "^$dev"; then
        udisksctl unmount --force -b "$dev" > /dev/null
    fi
    ...
    while [ $i -ne 0 ]; do
        ...
        if ! umount "${mnt}" > /dev/null 2>&1; then
            ...
            echo "The following processes are occupying ${mnt}:" >&2
            lsof ${mnt} 2>/dev/null
            return 1
        fi
        i=$(expr $i - 1)
...
generate_flash_package() {
    ...
    if [ $encrypt_rootpart -eq 1 ]; then
        echo "encrypt-rootpart ${ROOTFS_DEVICE}p1" >> "$mnt/flashpkg/conf/command_sequence"
        mkdir "$mnt/flashpkg/rootfs"
        cp rootfs.tar.zst "$mnt/flashpkg/rootfs"
    fi

    echo "resize-datapart ${ROOTFS_DEVICE}p15" >> "$mnt/flashpkg/conf/command_sequence"

    mkdir "$mnt/flashpkg/datafs"
    cp data.tar "$mnt/flashpkg/datafs"

    if [ $encrypt_datapart -eq 1 ]; then
        echo "encrypt-datapart ${ROOTFS_DEVICE}p15" >> "$mnt/flashpkg/conf/command_sequence"
    fi

    if [ x"$dockerapp_provision" = x"1" ]; then
        mkdir "$mnt/flashpkg/dockerapp-provision-flasher"
        cp dockerapp-provision-flasher.tar.zst "$mnt/flashpkg/dockerapp-provision-flasher"
        echo "dockerapp-provision $dockerapp_provision_args" >> "$mnt/flashpkg/conf/command_sequence"
    fi
...
if [ $encrypt_rootpart -eq 1 ]; then
    step_banner "Encrypting rootfs partition"
    sleep 30
fi
if [ $encrypt_datapart -eq 1 ]; then
    step_banner "Encrypting datafs partition"
    sleep 20
else
    step_banner "Resizing rootfs partition size to full disk"
    sleep 10
fi
if [ $dockerapp_provision -eq 1 ]; then
    step_banner "Provisioning docker app"
    sleep 30
fi

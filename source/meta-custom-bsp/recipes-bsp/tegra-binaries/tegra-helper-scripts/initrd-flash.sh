...
ARGS=$(getopt -n $(basename "$0") -l "usb-instance:,help,skip-bootloader,erase-nvme,encrypt-rootpart,dockerapp-provision:" -o "u:v:h" -- "$@")
if [ $? -ne 0 ]; then
    usage >&2
    exit 1
fi
...
generate_flash_package() {
	...
	echo "encrypt-rootpart ${ROOTFS_DEVICE}p1" >> "$mnt/flashpkg/conf/command_sequence"
 	...
 	echo "resize-rootpart ${ROOTFS_DEVICE}p1" >> "$mnt/flashpkg/conf/command_sequence"
    fi
